...
Each `image` defines an `image.sources` set of image sources. Type and format of image source is defined by `mimeType` and `internalFormat` specifiers.
...
These formats are usually with non-OpenGL APIs as well. See the following table for matches with Vulkan, D3D11/12, and Metal:
